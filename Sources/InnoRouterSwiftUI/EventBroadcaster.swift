...
// InnoRouterSwiftUI - MainActor-isolated multi-subscriber event fan-out
// Copyright © 2026 Inno Squad. All rights reserved.

import Foundation

/// Internal helper that fans a single event out to multiple
/// `AsyncStream` subscribers.
///
/// Each store (`NavigationStore`, `ModalStore`, `FlowStore`) owns one
/// broadcaster keyed by its event enum. Subscribers receive their own
/// `AsyncStream` via `stream()`, and the broadcaster cleans up
/// per-subscriber state through `AsyncStream.Continuation.onTermination`
/// so cancelled `for await` loops do not leak continuations.
///
...
/// down.
@MainActor
final class EventBroadcaster<Event: Sendable> {
